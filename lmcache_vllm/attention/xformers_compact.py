    ...
    def forward(
        ...
        * For cross-attention:
        ...
        if decode_meta := attn_metadata.decode_metadata:
            ...
            logits_store = torch.empty((self.num_heads, num_toks),
                               device=device,
                               dtype=torch.float32)
            output_compact = torch.empty_like(output[num_prefill_tokens:])
            ...
            # update buffers here
            lmcache_compactor = LMCacheCompactorBuilder.get("lmcache_compactor")
            lmcache_compactor.update_imp_scores_buffer(logits_store)
